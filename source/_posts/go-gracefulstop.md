...
Go 为我们提供了 `os/singal` 内置包用来处理信号，`os/singal` 包提供了如下 [6 个函数](https://pkg.go.dev/os/signal@go1.22.0#pkg-index) 供我们使用：
...
这是我迄今为止，写过的最长单篇文章了，希望读完此文的你，能一次将 Go 程序中的优雅退出理解透彻。本来只是想简单分享下 K8s 中优雅退出的思路，没想到越写越多，再不收手就真的控制不住了🤣。我差点要继续去深入研究 `os/singal`、`net/http`、`grpc`... 的源码，等以后有机会再单独分享吧 :)。
